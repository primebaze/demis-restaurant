...
export async function GET(_req: Request, { params }: Context) {
  ...
  try {
    const plan = await prisma.shootPlan.findUnique({
      where: { id: params.id },
      select: { editorShare: { select } },
    });
    return plan
      ? shareJson({ share: plan.editorShare })
...
export async function PUT(req: Request, { params }: Context) {
  ...
  try {
    ...
    if (!body.version) {
      ...
      try {
        const share = await prisma.shootPlanEditorShare.create({
          data: {
            planId: params.id,
    ...
    const share = await prisma.$transaction(async (tx) => {
      const changed = await tx.shootPlanEditorShare.updateMany({
      ...
      return tx.shootPlanEditorShare.findUniqueOrThrow({
